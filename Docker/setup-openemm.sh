...
then
	...
	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm_cms < openemm_cms-2015.sql
	echo -e "[OK]"

	echo -n "=> Create OpenEMM Database: "
	mysqladmin -f -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS drop openemm 2> /dev/null 1> /dev/null
	...
	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm < openemm-2015.w.sql
	echo -e "[OK]"

	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm -e 'CREATE INDEX custbind$cuid_mlid_user$idx ON customer_1_binding_tbl (customer_id, mailinglist_id, user_status, user_type);'
	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm -e 'CREATE INDEX onpx$mid_cuid$idx ON onepixel_log_tbl (mailing_id, customer_id);'
	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm -e 'CREATE INDEX rlog$mid_cuid_urlid$idx ON rdir_log_tbl (mailing_id, customer_id, url_id);'
else
	mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASS openemm -e "UPDATE company_tbl SET rdir_domain='$OPEN_EMM_URL', mailloop_domain='$OPEN_EMM_HOSTNAME';"
fi
...
sed -i "s/url=\"jdbc:mysql:\/\/localhost/url=\"jdbc:mysql:\/\/$MYSQL_HOST/g" /home/openemm/conf/context.xml
sed -i "s/username=\"agnitas\"/username=\"$MYSQL_USER\"/g" /home/openemm/conf/context.xml
sed -i "s/password=\"openemm\"/password=\"$MYSQL_PASS\"/g" /home/openemm/conf/context.xml
...
sed -i "s/'localhost', 'agnitas', 'openemm'/'$MYSQL_HOST', '$MYSQL_USER', '$MYSQL_PASS'/g" /home/openemm/bin/scripts/agn.py

sed -i "s/'BOUNCE_DOMAIN'/'$OPEN_EMM_BOUNCE_DOMAIN'/g" /etc/mail/sendmail.mc

su openemm -c '/home/openemm/bin/sendmail-enable.sh'

touch /var/log/maillog && chmod 604 /var/log/maillog
ln -s /usr/lib/x86_64-linux-gnu/libmilter.so.1.0.1 /usr/lib/libmilter.so.1.0
